...
FrameGraphId<FrameGraphTexture> PostProcessManager::gaussianBlurPass(FrameGraph& fg,
    ...
    // 5 positive-side samples, give 4+1+4=9 samples both sides
    ...
    auto computeGaussianCoefficients = [](float* coefs, float* offsets, size_t size, float alpha) -> size_t {
    ...
    auto& gaussianBlurPasses = fg.addPass<BlurPassData>("Gaussian Blur Passes",
            ...
            [=](FrameGraphPassResources const& resources,
                ...
                float coefs[kMaxPositiveKernelSize];
                float offsets[kMaxPositiveKernelSize];
                size_t m = computeGaussianCoefficients(coefs, offsets, kMaxPositiveKernelSize, alpha);
                ...
                mi->setParameter("kernel", coefs, m);
                mi->setParameter("offsets", offsets, m);
                mi->commit(driver);
                mi->use(driver);
